---
title: 搭配子點選分析使用產品層級細分
description: 瞭解如何在Adobe Analytics中使用子點選分析，以獲得更精確的分段和報告。
feature: Segmentation
role: User
level: Experienced
doc-type: Feature Video
last-substantial-update: 2026-08-11T00:00:00Z
jira: KT-22301
source-git-commit: c3fe1c5d91aea129e8ec7b816917ccdcb9abf876
workflow-type: tm+mt
source-wordcount: '114'
ht-degree: 6%

---

# 在Adobe Analytics中使用產品層級細分搭配子點選分析

Adobe Analytics中的傳統區段通常僅限於點選層級，若有多個產品、計畫或物件與單一事件相關聯，就可能帶來挑戰。 當分析師想要隔離特定專案的效能時，這可能會導致量度膨脹或歸因不準確。

此影片示範Adobe Analytics中的子點選分析如何提供更細微的分段功能。

如需其他資訊，請瀏覽此[文件](https://experienceleague.adobe.com/en/docs/analytics/components/segmentation/sub-hit){target="_blank"}。

>[!VIDEO](https://video.tv.adobe.com/v/3497075/?quality=12&learn=on)
